...
then
	...
	# create exon key
	EXONKEY=$(qsub -V -wd $output_dir/logs -q $queue -m a -M $email $memory_exonkey $script_path/bam2wig/exon_key.sh -e $exon_bed -c $capture_bed -o $output_dir/PatternCNV.Exon.Key.txt -b $bin_size -t $config -x $extension_buffer -s $split_size $merge_param)
	jobid_exonkey=$(echo $EXONKEY | cut -d ' ' -f3)

	# bam2wig
	for bam in $(cat $bams_config)
	do
		qsub -V -wd $output_dir/logs -q $queue -m a -M $email $memory_bam2wig -hold_jid $jobid_exonkey $script_path/bam2wig/bam2wig.sh -i $bam -o $output_dir -b $bin_size -m $min_mapping_qual -t $config -e $output_dir/PatternCNV.Exon.Key.txt $merge_param
	done
	echo $(date)
fi
